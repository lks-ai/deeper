 ...
 * subscripts (_), grouping with braces, scalable delimiters (\left...\right), and basic
 * commands (e.g. \sin, \cos, \alpha, etc.).
 *
 * Usage:
...
(function (root, factory) {
  ...
  // Mapping of common Greek symbols and other LaTeX commands to UTF‑8.
  var greekMapping = {
  ...
  function parseExpression(tokens) {
    var nodes = [];
    // Stop if we see a closing brace or a \right (which terminates a scalable group).
  ...
  function parseAtom(tokens) {
    ...
    // Process optional superscript or subscript modifiers.
  ...
  // New function to handle scalable groups delimited by \left and \right.
  function parseScalableGroup(tokens) {
    // After a \left token is consumed, skip any whitespace.
  ...
  function renderNode(node) {
    ...
    switch (node.type) {
      ...
      case "sqrt":
        ...
        break;
      case "command":
        if (greekMapping[node.value] !== undefined) {
          html += '<span class="fatex-greek">' + greekMapping[node.value] + '</span>';
        } else {
